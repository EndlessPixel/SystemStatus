...
async function updateRealTimeData() {
    if (!(await checkBackendStatus())) return; try {
        const response = await fetch(`${API_BASE}/real-time-data`); const data = await response.json(); const sampleInterval = getSampleInterval();
        ...
        const batteryInfoEl = document.getElementById('battery-info'); if (batteryInfoEl && data.battery_info) {
...
function updateCPUCores(coreUsages, withAnimation = false) {
    const container = document.getElementById('cpu-cores-container'); if (!container) return; if (coreUsages && coreUsages.length > 0) { cachedCpuCores = [...coreUsages]; } if (!coreUsages || coreUsages.length === 0) { container.innerHTML = `<p>${t('noCPUCores')}</p>`; cachedCpuCores = []; return; } const existingCoreBoxes = container.querySelectorAll('.core-box'); const existingCount = existingCoreBoxes.length; const newCount = coreUsages.length;
    if (existingCount !== newCount) {
        ...
        const coreBox = container.children[index]; if (!coreBox) return; const coreNumEl = coreBox.querySelector('.core-num'); if (coreNumEl) { coreNumEl.textContent = `${t('core')} ${index + 1}`; } const coreUsageEl = coreBox.querySelector('.core-usage');
        if (coreUsageEl) { if (withAnimation) { animateNumber(coreUsageEl, usage, true); } else { coreUsageEl.textContent = `${usage.toFixed(1)}%`; } } coreBox.className = 'core-box'; if (usage < 30) coreBox.classList.add('low'); else if (usage < 70) coreBox.classList.add('medium'); else coreBox.classList.add('high');
    });
